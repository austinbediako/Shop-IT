      ...
      <div className="col-span-1 overflow-auto bg-white shadow-lg p-4">
        ...
        <div className="flex justify-center">
          <span
            ...
            className="cursor-pointer px-4 py-2 text-white rounded-none-none"
          >
            View All
          </span>
        </div>
      ...
      <tr>
          ...
          {order.allProduct.map((item, index) => {
            return (
              <img
                ...
                src={item.id.pImages[0].match(/^https?:\/\//) ? item.id.pImages[0] : `${apiURL}/uploads/products/${item.id.pImages[0]}`}
                alt="Pic"
              />
            );
          })}
        ...
        <td className="p-2 text-center">
          {order.status === "Not processed" && (
            <span className="block text-red-600 rounded-none-none text-center text-xs px-2 font-semibold">
              {order.status}
            </span>
          )}
          {order.status === "Processing" && (
            <span className="block text-yellow-600 rounded-none-none text-center text-xs px-2 font-semibold">
              {order.status}
            </span>
          )}
          {order.status === "Shipped" && (
            <span className="block text-blue-600 rounded-none-none text-center text-xs px-2 font-semibold">
              {order.status}
            </span>
          )}
          {order.status === "Delivered" && (
            <span className="block text-green-600 rounded-none-none text-center text-xs px-2 font-semibold">
              {order.status}
            </span>
          )}
