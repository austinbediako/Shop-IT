    ...
    <Fragment>
      <div className="relative mt-16 bg-gray-100 border-2">
        {data.sliderImages.length > 0 ? (
          <img
            className="w-full"
            src={data.sliderImages[slide].slideImage.match(/^https?:\/\//) ? data.sliderImages[slide].slideImage : `${apiURL}/uploads/customize/${data.sliderImages[slide].slideImage}`}
            alt="sliderImage"
          />
        ) : (
          ""
        ...
        {data?.sliderImages?.length > 0 ? (
          <>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <a
                href="#shop"
                className="cursor-pointer box-border text-2xl bg-blue-600 hover:bg-blue-700 transition-colors text-white px-8 py-3 rounded-none font-semibold tracking-wide"
              >
                Shop Now
              </a>
            </div>
