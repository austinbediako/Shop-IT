...
const Product = () => {
  ...
  if (loading) {
    return (
      <div className="my-32 text-2xl text-center font-light tracking-widest text-gray-400">
        Loading...
      </div>
    );
  }
    ...
    <Fragment>
      <div className="grid grid-cols-2 md:grid-cols-1">
        {products.length > 0 ? (
          ...
          <div className="col-span-2 md:col-span-1 flex flex-col items-center justify-center py-20 px-4 w-full">
            <div className="flex flex-col items-center w-full max-w-md text-center">
              <div className="relative w-full mb-10 bg-gray-50 overflow-hidden">
                <img src="/no-wishlist.jpg" alt="Empty Wishlist" className="w-full h-auto object-cover opacity-95 hover:opacity-100 transition-opacity duration-500" />
                <div className="absolute inset-0 border border-gray-200 pointer-events-none"></div>
              </div>
              <h2 className="text-3xl md:text-4xl font-light tracking-widest text-[#1a1a1a] uppercase mb-4">Wishlist Empty</h2>
              <div className="w-12 h-1 bg-blue-600 mb-6"></div>
              <p className="text-gray-500 font-light text-lg leading-relaxed mb-8">
                You haven't saved any items yet. Discover our latest collections and find something you love.
              </p>
              <button onClick={() => history.push('/')} className="px-10 py-4 bg-[#1a1a1a] text-white text-sm font-bold uppercase tracking-widest hover:bg-blue-600 transition-colors duration-300">
                Explore Catalog
              </button>
            </div>
          </div>
